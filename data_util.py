...
data_dir = '/home/Zxdon/Desktop/DL/Zxdon/dataset/'
full_data_dir = data_dir + 'cifar-10-batches-py/data_batch_'
vali_dir = data_dir + 'cifar-10-batches-py/test_batch'
...
def read_in_images(address_list, shuffle=True, is_random_label=False):
    """
    ...
    :param shuffle: whether shuffle default True
    :param is_random_label: generate random label?
    ...
    #data_label = batch_iter(data, label, batch_size, epoch)
...
def prepare_train_data(padding_size):
    ...
    data, label = read_in_images(path_list, is_random_label=TRAIN_RANDOM_LABEL)

    # pad images to train data
    pad_width = ((0, 0), (padding_size, padding_size), (padding_size, padding_size), (0, 0))
    data = np.pad(data, pad_width=pad_width, mode='constant', constant_values=0)
    # crop and flip all train data
    print 'Crop and Flip'
    data = random_crop_and_flip(data, padding_size=padding_size)
...
def prepare_validate_data():
    ...
    '''
    validation_array, validation_labels = read_in_images([vali_dir], is_random_label=VALI_RANDOM_LABEL)
    validation_array = whitening_image(validation_array)

    return validation_array, validation_labels


def train_data_iter(data, label, batch_size = 128, num_epochs=20, shuffle=True):
    """
    create by Zxdon
    :param data: all image (50000, 32, 32, 3)
    :param label: the label of image (50000,)
    :param batch_size: batch size
    :param num_epochs: number of epochs at training stage
...
# train_data, train_label = prepare_train_data(2)
# data_iter = train_data_iter(train_data, train_label)
# count = 0
#
# for _ in data_iter:
#     count += 1
